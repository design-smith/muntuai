...
const SettingsProfile = () => {
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      try {
        // Fetch business (which includes personal and products)
        ...
        if (business) {
          ...
          setIndustries(business.industries || []);
          ...
          // Personal
          setPersonalName(business.personalName || '');
          setPersonalTitle(business.personalTitle || '');
  ...
  // Save/update logic for each section
  const saveBusiness = async (fields) => {
    if (!businessId) return;
    try {
      await fetch(`http://localhost:8000/businesses/${businessId}`, {
        method: 'PUT',
        headers: {
        ...
        {profileTab === 'business' && (
          <form className="settings-profile-form">
              ...
              <div>
                {editingName ? (
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    <input
                      value={businessName}
                      onChange={e => { setBusinessName(e.target.value); saveBusiness({ businessName: e.target.value }); }}
            ...
            <Autocomplete
              options={industries}
              value={industry}
              onChange={(_, v) => { setIndustry(v || ''); saveBusiness({ industry: v || '' }); }}
              renderInput={(params) => (
                <TextInput {...params} label="Industry" value={industry} onChange={e => { setIndustry(e.target.value); saveBusiness({ industry: e.target.value }); }} />
              )}
              freeSolo
              autoHighlight
              fullWidth
            ...
            <div style={{ marginBottom: '1.5rem' }}>
              ...
              {editingDescription ? (
                <LongTextInput
                  ...
                  onChange={e => { setCompanyDescription(e.target.value); saveBusiness({ companyDescription: e.target.value }); }}
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                {editingEmail ? (
                  <TextInput
                    label="Email"
                    value={companyEmail}
                    onChange={e => { setCompanyEmail(e.target.value); saveBusiness({ companyEmail: e.target.value }); }}
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                {editingPhone ? (
                  <TextInput
                    label="Phone"
                    value={companyPhone}
                    onChange={e => { setCompanyPhone(e.target.value); saveBusiness({ companyPhone: e.target.value }); }}
              ...
              {socialLinks.map((link, idx) => {
                ...
                return editingSocialIdx === idx ? (
                  <div key={idx} style={{ display: 'flex', alignItems: 'center', border: '1.5px solid #e3e8f0', borderRadius: 12, padding: '0.7rem 1.2rem', background: 'transparent', gap: 10, minWidth: 320 }}>
                    ...
                    <input
                      value={link.url}
                      onChange={e => { setSocialLinks(links => links.map((l, i) => i === idx ? { ...l, url: e.target.value } : l)); saveBusiness({ socialLinks: socialLinks }); }}
                  ...
                  <div key={idx} style={{ display: 'flex', alignItems: 'center', border: '1.5px solid #e3e8f0', borderRadius: 12, padding: '0.7rem 1.2rem', background: 'transparent', gap: 10, minWidth: 320, cursor: 'pointer' }} onClick={() => setEditingSocialIdx(idx)}>
                    ...
                    <CloseIcon style={{ color: 'var(--desert-sand)', cursor: 'pointer' }} onClick={e => { e.stopPropagation(); setSocialLinks(links => links.filter((_, i) => i !== idx)); saveBusiness({ socialLinks: socialLinks }); }} />
                  </div>
                );
              })}
              {addSocialMode ? (
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, minWidth: 320 }}>
                  <select
                    ...
                    onChange={e => { setNewSocial(s => ({ ...s, platform: e.target.value })); saveBusiness({ socialLinks: [...socialLinks, newSocial] }); }}
                  ...
                  <input
                    ...
                    value={newSocial.url}
                    onChange={e => { setNewSocial(s => ({ ...s, url: e.target.value })); saveBusiness({ socialLinks: [...socialLinks, newSocial] }); }}
                  ...
                  <button
                    ...
                    onClick={() => {
                      if (newSocial.platform && newSocial.url) {
                        ...
                        saveBusiness({ socialLinks: socialLinks });
                      }
                    }}
                  >Save</button>
                </div>
              ...
              {locations.map((loc, idx) => (
                editingLocationIdx === idx ? (
                  <div key={idx} style={{ display: 'flex', alignItems: 'center', border: '1.5px solid #e3e8f0', borderRadius: 12, padding: '0.7rem 1.2rem', background: 'transparent', gap: 10, minWidth: 320 }}>
                    <input
                      value={loc}
                      onChange={e => { setLocations(locs => locs.map((l, i) => i === idx ? e.target.value : l)); saveBusiness({ locations: locations }); }}
                  ...
                  <div key={idx} style={{ display: 'flex', alignItems: 'center', border: '1.5px solid #e3e8f0', borderRadius: 12, padding: '0.7rem 1.2rem', background: 'transparent', gap: 10, minWidth: 320, cursor: 'pointer' }} onClick={() => setEditingLocationIdx(idx)}>
                    ...
                    <CloseIcon style={{ color: 'var(--desert-sand)', cursor: 'pointer' }} onClick={e => { e.stopPropagation(); setLocations(locs => locs.filter((_, i) => i !== idx)); saveBusiness({ locations: locations }); }} />
                  </div>
                )
              ))}
              {addLocationMode ? (
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, minWidth: 320 }}>
                  <input
                    ...
                    value={newLocation}
                    onChange={e => { setNewLocation(e.target.value); saveBusiness({ locations: [...locations, e.target.value] }); }}
                    style={{ flex: 1, background: 'transparent', border: '1.5px solid #e3e8f0', color: '#fff', borderRadius: 8, padding: '0.4rem 1rem', fontSize: '1.1rem' }}
                  />
                  <button
                    type="button"
                    ...
                    onClick={() => {
                      if (newLocation) {
                        ...
                        saveBusiness({ locations: locations });
                      }
                    }}
                  >Save</button>
                </div>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 16 }}>
              {editingPersonalName ? (
                <TextInput
                  label="Full Name"
                  value={personalName}
                  onChange={e => { setPersonalName(e.target.value); saveBusiness({ personalName: e.target.value }); }}
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 16 }}>
              {editingPersonalTitle ? (
                <TextInput
                  label="Title"
                  value={personalTitle}
                  onChange={e => { setPersonalTitle(e.target.value); saveBusiness({ personalTitle: e.target.value }); }}
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 16 }}>
              {editingPersonalEmail ? (
                <TextInput
                  label="Email"
                  value={personalEmail}
                  onChange={e => { setPersonalEmail(e.target.value); saveBusiness({ personalEmail: e.target.value }); }}
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 16 }}>
              {editingPersonalPhone ? (
                <TextInput
                  label="Phone"
                  value={personalPhone}
                  onChange={e => { setPersonalPhone(e.target.value); saveBusiness({ personalPhone: e.target.value }); }}
              ...
              {products.map((prod, idx) => (
                ...
                >
                  ...
                  <div style={{ display: 'flex', gap: 16, alignItems: 'center', position: 'absolute', bottom: 16, right: 24, opacity: 0, transition: 'opacity 0.18s', pointerEvents: 'none' }} className="product-card-actions">
                    ...
                    <DeleteOutlineIcon style={{ color: '#ff5252', cursor: 'pointer' }} onClick={e => { e.stopPropagation(); setProducts(ps => ps.filter((_, i) => i !== idx)); saveBusiness({ products: products }); }} />
                  </div>
                  <style>{`
            ...
            {productModal.open && (
              <div style={{ position: 'fixed', top: 0, left: 0, width: '100vw', height: '100vh', background: 'rgba(20,20,26,0.45)', zIndex: 2000, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                <div style={{ background: '#181e36', borderRadius: 18, minWidth: 420, maxWidth: 520, padding: '2.2rem 2.2rem 1.2rem 2.2rem', boxShadow: '0 4px 32px 0 rgba(39,45,79,0.18)' }}>
                  ...
                  <form onSubmit={e => { e.preventDefault(); if (productModal.idx === null) { setProducts(ps => [...ps, productForm]); } else { setProducts(ps => ps.map((p, i) => i === productModal.idx ? productForm : p)); } setProductModal({ open: false, idx: null }); saveBusiness({ products: products }); }}>
                    <div style={{ marginBottom: 18 }}>
                      <div style={{ fontWeight: 600, color: '#fff', marginBottom: 6 }}>Name</div>
                      <input
                        value={productForm.name}
                        onChange={e => { setProductForm(f => ({ ...f, name: e.target.value })); saveBusiness({ products: products }); }}
                    ...
                    <div style={{ marginBottom: 18 }}>
                      ...
                      <textarea
                        ...
                        onChange={e => { setProductForm(f => ({ ...f, description: e.target.value })); saveBusiness({ products: products }); }}
                    ...
                    <div style={{ marginBottom: 18, display: 'flex', gap: 16 }}>
                      <div style={{ flex: 1 }}>
                        ...
                        <select
                          ...
                          onChange={e => { setProductForm(f => ({ ...f, pricingType: e.target.value, price: e.target.value === 'Quoted Price' ? '' : f.price })); saveBusiness({ products: products }); }}
                      ...
                      {productForm.pricingType !== 'Quoted Price' && (
                        <div style={{ flex: 1 }}>
                          ...
                          <input
                            ...
                            onChange={e => { setProductForm(f => ({ ...f, price: e.target.value })); saveBusiness({ products: products }); }}
                            style={{ width: '100%', background: 'rgba(39,45,79,0.7)', border: '1.5px solid #2a2e3e', borderRadius: 10, color: '#fff', fontSize: '1.08rem', padding: '0.7rem 1.2rem' }}
                            min="0"
                            step="0.01"
